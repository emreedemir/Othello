...
public class GamePlay  :MonoBehaviour{
    ...
    {
        // two piece for every player initliaze
        m_piecePool = FindObjectOfType<PiecePool>();
        player1.AddPiece(m_piecePool.TakeFromPool(Piece.PieceType.WhitePiece), new Position(x1_P1, y1_P1));
        player1.AddPiece(m_piecePool.TakeFromPool(Piece.PieceType.WhitePiece), new Position(x2_P1, y2_P1));

        player2.AddPiece(m_piecePool.TakeFromPool(Piece.PieceType.BlackPiece), new Position(x1_P2, y1_P2));
        player2.AddPiece(m_piecePool.TakeFromPool(Piece.PieceType.BlackPiece), new Position(x2_P2, y2_P2));
    }
